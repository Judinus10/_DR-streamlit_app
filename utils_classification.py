...
CKPT_PATH = str(APP_DIR / "outputs" / "models" / "classification" / "best_macro_f1_model.pt")
...
def preprocess(pil_img: Image.Image):
    ...
    aug = get_transform()(image=raw_rgb)
    x = aug["image"].unsqueeze(0)

    return x, raw_rgb, display_rgb
...
def load_model_cached():
    ...
    if head_missing:
        raise RuntimeError(
            "Classification checkpoint loaded but classifier head weights are missing.\n"
...
def predict(model, device, pil_img: Image.Image):
    x, raw_rgb, display_rgb = preprocess(pil_img)
    x = x.to(device)
...
def choose_target_layer(model: torch.nn.Module, preferred_name: str | None = None):
    ...
    if not convs:
        raise ValueError("No Conv2d layer found in classification model.")
