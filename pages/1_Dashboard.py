...
def handle_upload_overwrite(current_right_name, current_left_name):
    """
    If a new file is chosen in the same slot, treat it as overwrite:
    clear old predictions/results tied to previous upload state.
    """
    prev_right = st.session_state.last_uploaded_names.get("right", "")
    prev_left = st.session_state.last_uploaded_names.get("left", "")

    right_changed = prev_right != current_right_name
    left_changed = prev_left != current_left_name
...
with left:
    ...
    # Current uploaded names
    current_right_name = uploaded_right.name if uploaded_right else ""
    current_left_name = uploaded_left.name if uploaded_left else ""

    # Treat new selection in same slot as overwrite
    handle_upload_overwrite(current_right_name, current_left_name)
