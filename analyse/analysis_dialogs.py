...
def inject_pdf_dialog_error_styles():
    st.markdown(
        """
        <style>
        .pdf-form-error-text {
        ...
        .missing-pdf-patient-id div[data-testid="stTextInput"] input,
...
def validate_pdf_inputs() -> dict:
    report_title = normalize_text(st.session_state.get("pdf_report_title", ""))
    patient_name = normalize_text(st.session_state.get("pdf_patient_name", ""))
    clinician_name = normalize_text(st.session_state.get("pdf_clinician_name", ""))
    patient_id = normalize_text(st.session_state.get("pdf_patient_id", ""))
    institution_name = normalize_text(st.session_state.get("pdf_institution_name", ""))

    errors = {
        "report_title": not bool(report_title),
        "patient_name": not bool(patient_name),
        "clinician_name": not bool(clinician_name),
        "patient_id": not bool(patient_id),
        "institution_name": not bool(institution_name),
    }
    ...
    return {
        ...
        "notes": normalize_text(st.session_state.get("pdf_notes", "")),
        "errors": errors,
        "missing_fields": [
            label
            for key, label in [
                ...
                ("patient_name", "Patient name"),
                ("clinician_name", "Clinician / Examiner"),
                ("patient_id", "Patient ID"),
...
def render_pdf_report_dialog(
    ...
    def _dialog():
        inject_pdf_dialog_error_styles()

        if "pdf_missing_fields" not in st.session_state:
        ...
        if missing_fields:
            st.markdown(
                f"""
                <div class="pdf-form-warning-box">
                    Missing required data: {", ".join(missing_fields)}.<br>
                    Fill the highlighted fields and try again.
                </div>
                """,
        ...
        report_title_class = "missing-report-title" if "Report title" in missing_fields else ""
        st.markdown(f'<div class="{report_title_class} pdf-field-wrap">', unsafe_allow_html=True)
        st.text_input("Report title *", key="pdf_report_title")
        st.markdown("</div>", unsafe_allow_html=True)
        if "Report title" in missing_fields:
            st.markdown(
                '<div class="pdf-form-error-text">Report title is required.</div>',
                unsafe_allow_html=True,
            )
        ...
        with row1_col1:
            ...
            if "Patient name" in missing_fields:
                st.markdown(
                    '<div class="pdf-form-error-text">Patient name is required.</div>',
                    unsafe_allow_html=True,
                )

        with row1_col2:
            clinician_name_class = (
                "missing-pdf-clinician-name" if "Clinician / Examiner" in missing_fields else ""
            )
            st.markdown(f'<div class="{clinician_name_class} pdf-field-wrap">', unsafe_allow_html=True)
            st.text_input("Clinician / Examiner *", key="pdf_clinician_name")
            st.markdown("</div>", unsafe_allow_html=True)
            if "Clinician / Examiner" in missing_fields:
                st.markdown(
                    '<div class="pdf-form-error-text">Clinician / Examiner is required.</div>',
                    unsafe_allow_html=True,
                )
        ...
        with row2_col1:
            ...
            if "Patient ID" in missing_fields:
                st.markdown(
                    '<div class="pdf-form-error-text">Patient ID is required.</div>',
                    unsafe_allow_html=True,
                )

        with row2_col2:
            institution_class = (
                "missing-pdf-institution-name" if "Institution" in missing_fields else ""
            )
            st.markdown(f'<div class="{institution_class} pdf-field-wrap">', unsafe_allow_html=True)
            st.text_input("Institution *", key="pdf_institution_name")
            st.markdown("</div>", unsafe_allow_html=True)
            if "Institution" in missing_fields:
                st.markdown(
                    '<div class="pdf-form-error-text">Institution is required.</div>',
                    unsafe_allow_html=True,
                )
